...
class LibraryManagerTest {
    ...
    @BeforeEach
    void setUp() {
        // CRITICAL: Manually set IDs because the database (PostgreSQL) would generate them,
        // but in our unit tests, we're mocking the repository, so we must provide fake IDs.
        book1.setId(1L);
        book2.setId(2L);
        book3.setId(3L);

        // Create manager with ONLY the repository mock (BookStorage is GONE)
    ...
    void addBook_ValidInput_AddsBookAndSaves() {
        ...
        // Note: In a real DB, the ID would be set by PostgreSQL.
        // In our mock test, the ID is generated by the Book constructor (if it did),
        // but since we removed that, we just check it's null (or we could mock the ID generation).
        // Actually, the Book constructor still generates an ID if you use the old constructor...
        // WAIT! The Book constructor still has `this.id = BookIDGenerator.generateNextID()`.
        // Did you delete BookIDGenerator yet? If YES, this test will fail because the constructor throws an error.
        // Let's assume you DELETED BookIDGenerator. Then the Book constructor does NOT set an ID.
        // So captured.getId() will be null. That's fine for a unit test, but we should assert it.
        assertNull(captured.getId()); // The ID will be set by the database, not by the constructor.
